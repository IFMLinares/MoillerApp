...
export const fetchArticles = async (
  ...
  try {
    ...
    const articles = results.map((article: ApiArticle) => ({
      ...
      price: article.sa_art_precio?.[0]?.monto || 0, // Validar la existencia de sa_art_precio y asignar un valor predeterminado
      ...
      stock: article.sa_stock_almacen?.[0]?.stock || 0, // Validar la existencia de sa_stock_almacen
